...
# Expanding the Beam
def beam_expansion(article, summaries, k, beam_size):
    new_summaries = []
    for summary_index in range(len(summaries)):
        for sentence_index in np.unique(np.array([random.randint(0,len(article)-1) for i in range(beam_size)])):
...
def beam_search(article, summaries, k, beam_size):
    if len(summaries[0]) == k:
        ...
        summaries = beam_expansion(article, summaries, k, beam_size)
...
def decode_scores(scores_in, article, hyperparameters):
    ...
    # Initialize with random summaries of length 1
    initial_summaries = np.unique(np.array([random.randint(0,len(article)-1) for i in range(beam_size)]))
    # Use beam search to find the best summary
    best_summary = beam_search(article, initial_summaries, k, beam_size)[0]
